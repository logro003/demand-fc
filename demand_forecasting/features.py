...
def creating_avg_num_of_rides_last_week_feature(df: pd.DataFrame) -> pd.DataFrame:
    """Function for creating feature for average number of rides in the area"""
    ...
    for area in list(df.h3index_small.unique()):
        ...
        for row in range(0, len(area_df)):
            ...
            area_one_week_before_df = area_df[
                (area_df.date < this_date) & (area_df.date >= one_week_before_date)
            ]
            ...
            if len(area_one_week_before_df) < 7:
                ...
                avg_number_of_rides_one_week_before = (
                    area_one_week_before_df.num_of_rides.mean()
                )
            result.append([area, this_date, avg_number_of_rides_one_week_before])

    avg_num_of_rides_in_area_df = pd.DataFrame(
        result,
        columns=["h3index_small", "date", "avg_num_rides_in_area_one_week_before"],
    )
...
def generate_features_for_training(df: pd.DataFrame) -> pd.DataFrame:
    """Function for adding features to dataframe"""
    ...
    df_with_added_features = pd.merge(
        df, avg_num_of_rides_in_area_df, on=["h3index_small", "date"], how="left"
    )
...
def generate_features_for_inference(
    unique_h3index_df: pd.DataFrame, df: pd.DataFrame, weather_df: pd.DataFrame
) -> pd.DataFrame:
    """ Function for adding features to inference dataframe """
    ...
    avg_num_of_rides_per_area_df = (
        df_one_week_before_df.groupby("h3index_small")
        .agg({"num_of_rides": "mean"})
        .reset_index()
    )
    avg_num_of_rides_per_area_df.rename(
        columns={"num_of_rides": "avg_num_rides_in_area_one_week_before"}, inplace=True
    )
    ...
    # Lets assume here the weather forecast we get for next day is accurate,
    # and let's use the data we have in weather_data.csv for the 2020-08-31
    weather_at_prediciton_date_df = weather_df[weather_df.date == date_of_prediction]

    inference_df = pd.merge(
        inference_df,
        weather_at_prediciton_date_df,
        left_on="start_date",
        right_on="date",
        how="left",
    )
