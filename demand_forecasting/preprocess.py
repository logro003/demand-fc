...
def preprocessing(rides_df:pd.DataFrame,
    ...
    date_from = training_date - relativedelta(days=number_of_days_to_include_in_training+1)
    rides_df = rides_df[(rides_df.start_date >= date_from) & (rides_df.start_date <= date_to)]
    ...
    rides_per_area_and_day_df = pd.merge(rides_per_h3_and_day_df, all_combination_df, on=['h3index_small', 'start_date'], how='outer')
    rides_per_area_and_day_df.num_of_rides.fillna(0, inplace=True)
    rides_per_area_and_day_df.sort_values(by=['h3index_small', 'start_date'], inplace=True)
    rides_per_area_and_day_df = rides_per_area_and_day_df.reset_index().drop(columns='index')

    # Merging on weather per day 
    preprocessed_df = pd.merge(rides_per_area_and_day_df, weather_df, left_on='start_date', right_on='date', how='left')
